...
class Library {
  ...
  // Aggiunto il metodo removeBook mancante
  removeBook(id) {
    const index = this.books.findIndex((book) => book.id === id); // Trova l'indice del libro con quell'ID
    if (index !== -1) {
      this.books.splice(index, 1); // Rimuove il libro dall'array
    }
    this.render(); // Aggiorna la visualizzazione
  }
...
// --- Dati di esempio ---
library.addBook("Il mio giorno", "Luca", 23, true);
library.addBook("Harry Potter", "Giorgia", 23, true);
library.addBook("Lessico", "Antonio", 23, true);
library.addBook("Avocado", "Mina", 23, true);
library.addBook("Gatorade", "Renato", 23, true);
